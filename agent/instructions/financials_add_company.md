...
- Follow [financials.md](./financials.md): **`financials/README.md`** for fiscal calendar and naming; one **`YYYY-Qn.yaml`** (or fiscal equivalent) per period you add. Use **“Source processing order and incremental writes (agents)”**: process exhibits (and API batches) in **priority order** and **write each quarter YAML after each source step**, not only when the full quarter is finished.

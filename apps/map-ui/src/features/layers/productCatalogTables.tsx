...
const CHIPS_PRODUCT_ROWS: {
...
}[] = [
  {
    label: "Generation",
    description:
      "Vendor architecture generation name (e.g. Hopper, Blackwell, Rubin).",
    pick: (p) => p.generation,
    pickNotes: () => null,
    format: "text",
  },
  {
    label: "Status",
    description:
      "Product lifecycle stage: sampling, production, or end-of-life.",
    pick: (p) => p.status,
    pickNotes: () => null,
    format: "text",
  },
  {
    label: "Memory type",
    description:
      "High-bandwidth memory standard (e.g. HBM3, HBM3e, HBM4). Determines per-stack bandwidth and capacity.",
    pick: (p) => p.memoryType,
  ...
  },
  {
    label: "Memory (GB)",
    description:
      "Total GPU memory capacity in gigabytes. Larger memory enables bigger models, batch sizes, and KV caches.",
  ...
  },
  {
    ...
    description:
      "Peak memory bandwidth in terabytes per second. Key bottleneck for memory-bound inference workloads.",
  ...
  },
  {
    label: "FP8 (TFLOPS)",
    description:
      "Peak 8-bit floating-point throughput in tera-FLOPS. Primary metric for LLM training and inference. Check notes for dense vs. sparse.",
  ...
  },
  {
    label: "FP16 (TFLOPS)",
    description:
      "Peak 16-bit (FP16/BF16) floating-point throughput in tera-FLOPS. Used in mixed-precision training workflows.",
  ...
  },
  {
    label: "FP32 (TFLOPS)",
    description:
      "Peak 32-bit single-precision throughput in tera-FLOPS. Non-tensor-core performance; relevant for HPC and scientific workloads.",
  ...
  },
  {
    label: "TDP (W)",
    description:
      "Thermal design power in watts — the maximum sustained power draw per GPU. Determines cooling requirements and rack density.",
    pick: (p) => p.tdpWatts,
  ...
  },
  {
    label: "FP8 FLOPS/W",
    description:
      "Energy efficiency: peak FP8 TFLOPS divided by TDP watts. Higher is better — directly affects operating cost per FLOP.",
  ...
  },
  {
    label: "Interconnect",
    description:
      "GPU-to-GPU link technology and bandwidth within a node (e.g. NVLink, Infinity Fabric). Critical for multi-GPU model parallelism.",
  ...
  },
  {
    label: "Training",
    description:
      "Workload fitness for model training: strong, competitive, limited, or not applicable.",
  ...
  },
  {
    label: "Inference",
    description:
      "Workload fitness for model inference: strong, competitive, limited, or not applicable.",
  ...
  },
  {
    label: "Fine-tuning",
    description:
      "Workload fitness for model fine-tuning: strong, competitive, limited, or not applicable.",
...
const MODELS_PRODUCT_ROWS: {
...
}[] = [
  {
    label: "Status",
    description:
      "Product lifecycle stage: preview, ga (generally available), or deprecated.",
    pick: (p) => p.status,
    pickNotes: () => null,
    format: "text",
  },
  {
    ...
    description:
      "Whether model weights are proprietary, released as open weights, or fully open source.",
  ...
  },
  {
    label: "Quality index",
    description:
      "Artificial Analysis composite quality score (0–100). Blends reasoning, coding, math, and instruction following. Higher = better overall model.",
  ...
  },
  {
    label: "GPQA Diamond (%)",
    description:
      "% correct on GPQA Diamond (PhD-level science/reasoning). Random baseline ~25%. Key frontier reasoning signal.",
  ...
  },
  {
    ...
    description:
      "% of real GitHub issues resolved end-to-end. Best available coding/engineering benchmark.",
  ...
  },
  {
    ...
    description:
      "API price per million input tokens at standard (non-batch) tier. Snapshot date shown in notes.",
  ...
  },
  {
    ...
    description:
      "API price per million output tokens — the primary cost driver for high-volume deployments.",
  ...
  },
  {
    ...
    description:
      "Derived: quality_index ÷ output price per million tokens. Higher = better price-performance. Recomputed when pricing changes.",
  ...
  },
  {
    label: "TTFT (ms)",
    description:
      "Median time to first token in milliseconds (Artificial Analysis). Drives perceived responsiveness in interactive apps.",
  ...
  },
  {
    ...
    description:
      "Sustained output throughput in tokens per second (Artificial Analysis). Key for batch and agentic workloads.",
...
function formatCtx(v: number | null): string {
  ...
  if (v >= 1_000)
    return `${(v / 1_000).toLocaleString(undefined, { maximumFractionDigits: 0 })}K`;
  return v.toLocaleString();
}
...
export type ProductCatalogTableMode = "layer" | "simple";

type ChipsProps = {
  products: ChipsProductView[];
...
export function ChipsProductsCompareTable({
  ...
  if (mode === "simple") {
    return (
      <div
        className="simple-map__product-catalog"
        aria-label="Accelerator product specifications"
      >
        {table}
      </div>
    );
...
type ModelsProps = {
  products: ModelsProductView[];
  mode?: ProductCatalogTableMode;
};

export function ModelsProductsCompareTable({
  products,
  mode = "layer",
}: ModelsProps) {
  if (products.length === 0) return null;
  ...
  if (mode === "simple") {
    return (
      <div
        className="simple-map__product-catalog"
        aria-label="Model product specifications"
      >
        {table}
      </div>
    );
